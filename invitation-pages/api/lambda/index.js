...
exports.handler = async (event) => {
  ...
  try {
    ...
    if(!SENDER || !RECEIVER) {
      return { statusCode: 500, headers: corsHeaders, body: JSON.stringify({ error: 'SENDER_EMAIL and RECEIVER_EMAIL env vars must be set' }) };
    }
    ...
    return { statusCode: 500, headers: corsHeaders, body: JSON.stringify({ error: err.message || 'unknown' }) };
  }
};
